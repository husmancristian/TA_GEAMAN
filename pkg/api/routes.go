	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/jobs", func(r chi.Router) {
			...
			r.Route("/{jobId}", func(r chi.Router) {
				...
				r.Post("/prioritize", api.HandlePrioritizeJob)      // Re-queue a PENDING job with highest priority
				r.Get("/status-check", api.HandleGetJobStatusCheck) // Get current status of a job
